...
func newMemDB(t *testing.T) *gorm.DB {
    ...
    if err := AutoMigrate(db); err != nil { t.Fatalf("migrate: %v", err) }
    return db
}
...
func TestBumpSOASerialAuto_CreatesDefaultSOA(t *testing.T) {
    ...
    var cnt int64
    if err := db.Model(&RRSet{}).Where("zone_id = ? AND type = ?", z.ID, "SOA").Count(&cnt).Error; err != nil { t.Fatalf("count: %v", err) }
    if cnt != 0 { t.Fatalf("expected no SOA, got %d", cnt) }
    ...
    if soa.TTL != 3600 { t.Fatalf("unexpected TTL: %d", soa.TTL) }
    if len(soa.Records) != 1 { t.Fatalf("expected 1 soa record, got %d", len(soa.Records)) }
    parts := strings.Fields(soa.Records[0].Data)
    if len(parts) < 7 { t.Fatalf("invalid SOA rdata: %q", soa.Records[0].Data) }
    ...
    if len(parts2) < 7 { t.Fatalf("invalid SOA rdata after bump: %q", soa2.Records[0].Data) }
    ...
    if n2 <= n1 { t.Fatalf("serial did not increase: %d -> %d", n1, n2) }
}
